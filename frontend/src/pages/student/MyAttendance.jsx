  ...
  if (!loading && !userProfile?.student_id) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-center">
        <div className="w-20 h-20 rounded-[32px] bg-warning/10 flex items-center justify-center mb-6 border border-warning/20">
          <LayoutDashboard className="w-10 h-10 text-warning" />
        </div>
  ...
  return (
    <div className="space-y-12 pb-20 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2 text-[13px] font-medium text-fg-tertiary mb-2">
          <span>Overview</span>
          <ChevronRight className="w-3 h-3" />
          <span className="text-fg-secondary">My Attendance</span>
        </div>
        <div className="flex items-center gap-4">
           <TrendingUp className="w-6 h-6 text-accent-glow" />
      ...
      {loading ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <Skeleton className="lg:col-span-2 h-[220px] rounded-[48px]" />
          <Skeleton className="h-[220px] rounded-[48px]" />
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* Main Progress Card */}
          <div className="lg:col-span-2 bg-surface-raised/40 backdrop-blur-xl border border-white/10 rounded-[48px] p-10 flex flex-col md:flex-row items-center gap-10 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.5)]">
            <CircularProgress percentage={stats.percentage} />
            <div className="flex-1 text-center md:text-left">
              <h3 className="text-[28px] font-bold text-fg-primary mb-3">
                {stats.percentage >= 75 ? "You're doing great!" : "Focus required!"}
              </h3>
          ...
          <div className="bg-surface-raised/40 backdrop-blur-xl border border-white/10 rounded-[48px] p-10 flex flex-col justify-center shadow-[0_32px_64px_-16px_rgba(0,0,0,0.5)] group hover:border-accent-glow/30 transition-colors">
      ...
      <div className="space-y-8 mt-4">
        ...
        {loading ? (
              ...
              >
                <div className="flex items-center gap-6">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300 ${
                    record.present ? 'bg-success/10 text-success' : 'bg-danger/10 text-danger'
                  }`}>
                ...
                <div className="flex items-center gap-10">
                   <div className={`hidden md:block px-5 py-2 rounded-xl text-[11px] font-black tracking-widest border transition-all duration-300 ${
                     record.present 
                   ...
                   <button className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center text-fg-tertiary group-hover:bg-accent-glow group-hover:text-white transition-all duration-300">
          ...
          <div className="bg-surface-raised/20 border border-dashed border-white/10 rounded-[48px] py-32 flex flex-col items-center justify-center text-center">
            <div className="w-20 h-20 rounded-[32px] bg-white/5 flex items-center justify-center mb-6">
               <Clock className="w-10 h-10 text-fg-tertiary/20" />
            </div>
            <h3 className="text-xl font-bold text-fg-primary mb-2">No Records Yet</h3>
